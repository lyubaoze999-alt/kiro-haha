  ...
  const handleChooseFolder = async () => {
    if (isTauriRuntime()) {
      try {
        ...
        // Native dialog unavailable/failed → fall back to in-app browse below
        console.error('[DirectoryPicker] native folder dialog failed, falling back to browse:', err)
      }
    }
    // Web browser or Tauri dialog failure: directory tree via backend API
    setMode('browse')
