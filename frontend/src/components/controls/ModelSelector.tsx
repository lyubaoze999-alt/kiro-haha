...
function buildProviderChoices(
  ...
  for (const provider of providers) {
    ...
    choices.push({
      ...
      isDefault: isActive,
      models: isActive && availableModels.length > 0
        ? availableModels
...
export function ModelSelector({
  ...
  // Kiro CLI ACP has no reasoning-effort/thinking control, so the effort selector is hidden.
  const canEditRuntimeEffort = false
