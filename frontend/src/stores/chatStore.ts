  ...
  handleServerMessage: (sessionId, msg) => {
    ...
    switch (msg.type) {
      ...
      case 'message_complete': {
        ...
        const turnCredits = msg.usage && typeof (msg.usage as { credits?: number }).credits === 'number' ? (msg.usage as { credits?: number }).credits! : 0
        let completionMessages = session.messages
        if (text.trim()) {
          completionMessages = appendAssistantTextMessage(session.messages, text, Date.now())
          if (turnCredits > 0) {
            const li = completionMessages.length - 1
            const lm = completionMessages[li]
            if (lm?.type === 'assistant_text') completionMessages = [...completionMessages.slice(0, li), { ...lm, credits: turnCredits }]
          }
          update(() => ({
